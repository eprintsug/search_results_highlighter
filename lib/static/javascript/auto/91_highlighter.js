...
// get search query for advanced search
...
function highlight_search_results(){
    const advanced_fields = getAdvancedSearchParams();
    ...
    if (is_advanced_search()){
        advanced_fields.forEach(function(field){
            put_highlighted_span(
                ...
                search_keywords[field]["kw_array"]);
        });
    }
...
function put_highlighted_span(elements, keywords_array){
    ...
    elements.each( function(idx, elem) {
        let node = elem;
        keywords_array.forEach( function(kw){ 
...
function augment_links(){
    ...
    jQuery(".ep_search_result a")
        .map(function(){
            jQuery(this).attr('href', jQuery(this).attr("href") + q_params );
        })
}
...
function simplify_advanced_params(){
    ...
    const advanced_fields = ["title", "contributors_name%2Fcorp_creators%2Fcreators_name%2Feditors_name", "abstract"];
    let params_list =[]; 
    advanced_fields.map(function(x){
        return get_search_params(x);
    }).each(function(z){
            z.each(function(y){
                params_list.push(y);})});
    let simplified_params = "?q=";
    params_list.each(function(x) {
        if (x) {
            simplified_params += x + "+";
        }});
    //strip away the last &
 ...
 * Unfortunately I cannot use lambdas because it breaks compatibility with IE.
...
function remove_stopwords(arr){
    ...
    return arr
        ...
        .filter(function(x) {return stopwords.indexOf(x) === -1;});
}
...
function get_search_keywords_dict(search_fields){
    let search_keywords = {};
    search_fields.map(function(field) {
...
function get_search_params(search_field){
    ...
    url_str.replace(re, 
            function(string, fieldname, my_params){
